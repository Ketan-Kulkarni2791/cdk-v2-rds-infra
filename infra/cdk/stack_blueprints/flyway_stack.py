...
class FlywayStack(Stack):
    ...
    def __init__(self, env_var: str, scope: Construct, 
                 app_id: str, config: dict, rdsStack: RDSStack,**kwargs: Dict[str, Any]) -> None:
        """Creates the cloudformation templates for the projects."""
        super().__init__(scope, app_id, **kwargs)
        self.env_var = env_var
        self.config = config
        FlywayStack.create_stack(self, self.env_var, config=config, rdsStack=rdsStack)
    ...
    def create_stack(stack: aws_cdk.Stack, env: str, config: dict) -> None:
        ...
        FlywayStack.setup_athena(stack, RDSStack, rds_endpoint, rds_secret_arn)

    def setup_athena(stack: aws_cdk.Stack, RDSStack: RDSStack, rds_endpoint, rds_secret_arn):
        """Athena Infra setup."""
